...
const ItemDetailContainer = () => {
    ...
    useEffect(() => {
      let promesaProductos = new Promise((res, rej) => {
        setTimeout(() => {
          res([
            { id: 100, name: 'Campera Adidas', price: 20000, category:'nuevo', description:'Esta prenda está confeccionada con Primeblue, un material reciclado de alto rendimiento que contiene Parley Ocean Plastic y un mínimo de 40 % de material reciclado en total.', img:'https://i.ibb.co/Gtrbhpd/campera-Adidas.webp', stock:5 },
      ...
      promesaProductos
        .then((res) => {
          setProductDetail(res.find((item) =>item.id===100));
        })
        .catch((err) => {
          setError(err);
        })
